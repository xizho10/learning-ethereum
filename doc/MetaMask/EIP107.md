...
![Alt text](../../img/MetaMask/CORS_principle.png)
***
...
这允许`Dapp`连接到节点的`rpc api`而不被授予任何类型的权限。这样，用户可以在账户未解锁的情况下，安全地与日常Web浏览器中运行的`Dapp`进行交互。如果帐户未解锁，并且节点已通过rpc允许“个人”API，则html页面还允许用户输入其密码以解锁帐户以获得交易范围。
